...
local funcBufferMngr = Buffer.new(200, "player")



local function _namestiSalone()
...
addEventHandler("onResourceStart", resourceRoot, _namestiSalone)


local function _posaljiSalonInfo(cekaju)
    local zaClient = {}
...
funcBufferMngr:setHandler(_posaljiSalonInfo)

addEvent("onClientTraziSalonInfo", true)
addEventHandler("onClientTraziSalonInfo", root, function()
    funcBufferMngr:appendElement(client)
end)

local function _voziloUPonudi(salonIdx, voziloModelID)
...
local function _voziloKupovina(salonIdx, voziloModelID)
    
    if not _voziloUPonudi(salonIdx, voziloModelID) then
...
addEventHandler("onVoziloKupovina", resourceRoot, _voziloKupovina)